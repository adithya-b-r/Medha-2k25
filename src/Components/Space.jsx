...
export default function Space(props) {
  ...
  const ref = useRef()
  const { nodes } = useGLTF("/models/space/space.gltf")
  const { position, rotation, scale } = useControls("Space", {
  ...
  const [positions, colors] = useMemo(() => {
    ...
    for (let i = 0; i < positions.length; i += 3) {
      ...
      color.setRGB(redIntensity, greenIntensity, blueIntensity)
  ...
  return (
    <group ref={wrapper} {...props} position={position} rotation={rotation} scale={scale} dispose={null}>
      <group ref={ref}>
        ...
        <Points scale={0.05} positions={positions} colors={colors}>
          <pointsMaterial transparent depthWrite={false} vertexColors opacity={0.9} depthTest size={0.012} sizeAttenuation blending={THREE.AdditiveBlending} />
        </Points>
      </group>
    </group>
  )
...
useGLTF.preload("/models/space/space.gltf")
